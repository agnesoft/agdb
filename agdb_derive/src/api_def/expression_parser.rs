...
use crate::api_def::generics_parser::Generic;
...
/// Parse a block into an `::agdb::api_def::Expression::Block(...)`.
...
pub(crate) fn parse_expression(e: &Expr, generics: &[Generic]) -> TokenStream2 {
    match e {
        ...
        Expr::Break(_) => quote! { ::agdb::api_def::Expression::Break },
        ...
        Expr::Continue(_) => quote! { ::agdb::api_def::Expression::Continue },
        ...
        Expr::Infer(_) => quote! { ::agdb::api_def::Expression::Wild },
    ...
    match stmt {
        ...
        Stmt::Expr(expr, semi) => {
            ...
            if last && semi.is_none() && is_returnable(expr) {
                quote! { ::agdb::api_def::Expression::Return(Some(&#parsed)) }
            } else {
                parsed
            }
        }
        Stmt::Item(_) => quote! { ::agdb::api_def::Expression::Block(&[]) },
        ...
        .map(|gt| {
            gt.args
                ...
                .filter_map(|ga| match ga {
                    GenericArgument::Type(ty) => Some(quote! { <#ty as ::agdb::api_def::TypeDefinition>::type_def }),
                    _ => None,
                })
                .collect::<Vec<_>>()
        })
...
fn parse_closure(e: &ExprClosure, generics: &[Generic]) -> TokenStream2 {
    ...
    let body = match e.body.as_ref() {
        ...
        other => {
            ...
            vec![quote! { ::agdb::api_def::Expression::Return(Some(&#expr)) }]
        }
    };
...
fn parse_closure_arg(pat: &Pat, generics: &[Generic]) -> (TokenStream2, TokenStream2) {
    match pat {
        ...
        Pat::Ident(p) => {
            let name = &p.ident;
            (quote! { #name }, quote! { <() as ::agdb::api_def::TypeDefinition>::type_def })
        }
        _ => panic!(
...
fn parse_return_type(ret: &ReturnType, generics: &[Generic]) -> TokenStream2 {
    match ret {
        ReturnType::Default => quote! { <() as ::agdb::api_def::TypeDefinition>::type_def },
...
fn parse_loop(e: &syn::ExprLoop, generics: &[Generic]) -> TokenStream2 {
    ...
    quote! {
        ::agdb::api_def::Expression::While {
            condition: &::agdb::api_def::Expression::Literal(::agdb::api_def::LiteralValue::Bool(true)),
            body: &#body,
        }
    }
}
...
fn parse_literal(lit: &Lit) -> TokenStream2 {
    match lit {
        Lit::Str(s) => {
            let value = s.value();
            quote! {
                ::agdb::api_def::Expression::Literal(::agdb::api_def::LiteralValue::Str(#value))
            }
        }
        Lit::Int(i) => {
            let suffix = i.suffix();
            match suffix {
                "i8" => {
                    ...
                    quote! { ::agdb::api_def::Expression::Literal(::agdb::api_def::LiteralValue::I8(#v)) }
                }
                "i16" => {
                    let v = i.base10_parse::<i16>().unwrap();
                    quote! { ::agdb::api_def::Expression::Literal(::agdb::api_def::LiteralValue::I16(#v)) }
                }
                "i32" | "" => {
                    let v = i.base10_parse::<i32>().unwrap();
                    quote! { ::agdb::api_def::Expression::Literal(::agdb::api_def::LiteralValue::I32(#v)) }
                }
                "u8" => {
                    let v = i.base10_parse::<u8>().unwrap();
                    quote! { ::agdb::api_def::Expression::Literal(::agdb::api_def::LiteralValue::U8(#v)) }
                }
                "u16" => {
                    let v = i.base10_parse::<u16>().unwrap();
                    quote! { ::agdb::api_def::Expression::Literal(::agdb::api_def::LiteralValue::U16(#v)) }
                }
                "u32" => {
                    let v = i.base10_parse::<u32>().unwrap();
                    quote! { ::agdb::api_def::Expression::Literal(::agdb::api_def::LiteralValue::U32(#v)) }
                }
                "u64" => {
                    let v = i.base10_parse::<u64>().unwrap();
                    quote! { ::agdb::api_def::Expression::Literal(::agdb::api_def::LiteralValue::U64(#v)) }
                }
                "usize" => {
                    let v = i.base10_parse::<usize>().unwrap();
                    quote! { ::agdb::api_def::Expression::Literal(::agdb::api_def::LiteralValue::Usize(#v)) }
                }
        ...
        Lit::Float(f) => {
            ...
            match suffix {
                "f32" => {
                    ...
                    quote! { ::agdb::api_def::Expression::Literal(::agdb::api_def::LiteralValue::F32(#v)) }
                }
                "f64" | "" => {
                    let v = f.base10_parse::<f64>().unwrap();
                    quote! { ::agdb::api_def::Expression::Literal(::agdb::api_def::LiteralValue::F64(#v)) }
                }
        ...
        Lit::Bool(b) => {
            let value = b.value;
            quote! {
                ::agdb::api_def::Expression::Literal(::agdb::api_def::LiteralValue::Bool(#value))
            }
        }
        Lit::Char(c) => {
    ...
    while let Some(c) = chars.next() {
            ...
            && let Some(next) = chars.next()
        {
            ...
            if next == '}' {
                ...
                args.push(quote! { ::agdb::api_def::Expression::Ident(#ident) });
            }
        }
    }
...
fn parse_path(path: &Path) -> TokenStream2 {
    ...
    if path.segments.len() == 1 && matches!(first.arguments, PathArguments::None) {
        ...
        return quote! {
            ::agdb::api_def::Expression::Ident(stringify!(#ident))
        };
    }
...
fn parse_try(e: &ExprTry, generics: &[Generic]) -> TokenStream2 {
    let expr = parse_expression(&e.expr, generics);
    quote! {
        ::agdb::api_def::Expression::Try(&#expr)
    }
}
...
fn parse_pattern(pat: &Pat, generics: &[Generic]) -> (TokenStream2, TokenStream2) {
    match pat {
        Pat::Ident(p) => {
            let name = &p.ident;
            (
                quote! { ::agdb::api_def::Expression::Ident(stringify!(#name)) },
                quote! { None },
            )
        }
        Pat::Lit(p) => {
        ...
        Pat::Slice(p) => {
            ...
            (
                quote! { ::agdb::api_def::Expression::Array(&[#(#elems),*]) },
                quote! { None },
            )
        }
        Pat::Struct(p) => {
        ...
        Pat::Tuple(p) => {
            let elems = p.elems.iter().map(|elem| parse_pattern(elem, generics).0);
            (
                quote! { ::agdb::api_def::Expression::Tuple(&[#(#elems),*]) },
                quote! { None },
            )
        }
        Pat::TupleStruct(p) => {
