        ...
        impl TryFrom<agdb::QueryResult> for #name {
            ...
            fn try_from(value: ::agdb::QueryResult) -> ::std::result::Result<Self, Self::Error> {
                ...
                value
                    .elements
                    .first()
                    .ok_or(Self::Error::new(::agdb::DbErrorKind::NotEnoughData, "No element found"))?
                    .try_into()
            }
        }
    };
...
fn impl_from_db_element(f: &syn::Field) -> Option<proc_macro2::TokenStream> {
    if let Some(name) = &f.ident {
                    ...
                    ::std::option::Option::None })
                        .map_or_else(|| ::std::result::Result::Ok(::std::option::Option::None), |v| {
                            if let ::std::result::Result::Ok(v) = v {
                                ...
                                ::std::result::Result::Err(::agdb::DbError::new(::agdb::DbErrorKind::TypeError, &format!("Failed to convert value of '{}': {}", #str_name, v.unwrap_err())))
                            }
                        })?
            });
        }
        ...
        return Some(quote! {
            ...
            .ok_or(::agdb::DbError::new(::agdb::DbErrorKind::NotFound, format!("Key '{}' not found", #str_name)))?
            .map_err(|e| ::agdb::DbError::new(::agdb::DbErrorKind::TypeError, &format!("Failed to convert value of '{}': {}", #str_name, e)))?
        });
    }
