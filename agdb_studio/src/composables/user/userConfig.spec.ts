...
describe("userConfig.ts", () => {
    ...
    describe("userColumns", () => {
        it("returns the user columns", () => {
            ...
            expect(
                userColumns.some((column) => column.key === "username"),
            ).toBe(true);
            expect(userColumns.some((column) => column.key === "admin")).toBe(
                true,
            );
            expect(userColumns.some((column) => column.key === "login")).toBe(
                true,
            );
        });
    });
    ...
    describe("userActions", () => {
        it("returns the user actions", () => {
            ...
            expect(userActions.some((action) => action.key === "logout")).toBe(
                true,
            );
            expect(userActions.some((action) => action.key === "delete")).toBe(
                true,
            );
        });
