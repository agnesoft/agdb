...
description: "Deploy agdb_server cluster on Kubernetes: StatefulSet, secrets, ConfigMap, TLS, and service routing"
---

The `agdb_server` can be run in Kubernetes using official Docker image. Optionally you can [build the image](https://github.com/agnesoft/agdb/blob/main/agdb_server/containerfile) yourself and host it at the place of your choosing. Please refer to the [server-docker](/docs/guides/how-to-run-server/server-docker) guide for available images.
