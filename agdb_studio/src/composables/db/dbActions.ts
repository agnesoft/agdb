...
import type { DbResource, DbType, DbUserRole } from "agdb_api/dist/openapi";
...
export const dbList = async (): Promise<AxiosResponse | undefined> => {
    if (shouldRunAdminAction.value) {
...
export const dbRestore = async (
    params: DbIdentification & { backup: string },
): Promise<AxiosResponse | undefined> => {
    if (shouldRunAdminAction.value) {
