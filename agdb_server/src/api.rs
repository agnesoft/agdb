...
QueryBuilder::insert().edges().from(vec![1, 2]).to(vec![2, 3]).each().values([[("k", 1).into()], [("k", 2).into()]]).query(),
QueryBuilder::insert().edges().from(vec![1, 2]).to(vec![2, 3]).each().values_uniform([("k", 1).into(), (1, 10).into()]).query(),
...
QueryBuilder::remove().values(["k1", "k2"]).ids(vec![1, 2]).query(),
...
QueryBuilder::select().values(["k", "k2"]).ids(vec![1, 2]).query(),
