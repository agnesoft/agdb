...
<Callout>
The certificate must be issued (or used as an alternative name - SAN) for the DNS name of the server used in its configuration `address` field. E.g. `agdb.default.svc.cluster.local`.
</Callout>
