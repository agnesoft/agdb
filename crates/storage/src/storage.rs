use crate::{
    storage_data::StorageData, storage_record::StorageRecord,
    storage_record_with_index::StorageRecordWithIndex, write_ahead_log_record::WriteAheadLogRecord,
};
use db_error::DbError;
use serialize::Serialize;

pub struct Storage<T: StorageData> {
    pub(crate) data: T,
}
...
impl<T: StorageData> Storage<T> {
    ...
    pub fn insert<V: Serialize>(&mut self, value: &V) -> Result<i64, DbError> {
        self.transaction();
    ...
    pub fn resize_value(&mut self, index: i64, new_size: u64) -> Result<(), DbError> {
        if new_size == 0 {
    ...
    pub fn value<V: Serialize>(&mut self, index: i64) -> Result<V, DbError> {
        let record = self.data.record(index)?;
        V::deserialize(&self.read(Self::value_position(record.position, 0), record.size)?)
    }

    pub fn value_at<V: Serialize>(&mut self, index: i64, offset: u64) -> Result<V, DbError> {
        let record = self.data.record(index)?;
        let bytes = self.read(
    ...
    pub fn value_size(&self, index: i64) -> Result<u64, DbError> {
