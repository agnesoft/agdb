...
#[cfg_attr(feature = "api", agdb::test_def())]
pub async fn file_to_memory() -> Result<(), TestError> {
    let mut server = TestServer::new().await?;
    let owner = &next_user_name();
    let db = &next_db_name();
    server.api.user_login(ADMIN, ADMIN).await?;
...
pub async fn memory_to_mapped_preserves_backup_and_audit() -> Result<(), TestError> {
    ...
    server.api.db_convert(owner, db, DbKind::Mapped).await?;
...
pub async fn file_to_memory_preserves_backup_and_audit() -> Result<(), TestError> {
    ...
    server.api.db_convert(owner, db, DbKind::Memory).await?;
    server.api.db_restore(owner, db).await?;
