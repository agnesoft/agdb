...
#[openapi(
    ...
    components(schemas(
        ...
        routes::admin::SetLogLevelRequest,
        routes::user::LogoutQuery,
...
QueryBuilder::insert().amend([[("k", 1).into()], [("k", 2).into()]]).ids([1, 2]).query(),
QueryBuilder::insert().amend_uniform([("k", 1).into()]).ids([1, 2]).query(),
...
QueryBuilder::remove().amend([[("k", 1).into()], [("k", 2).into()]]).ids([1, 2]).query(),
QueryBuilder::remove().amend_uniform([("k", 1).into()]).ids([1, 2]).query(),
QueryBuilder::remove().amend_uniform([("k", 1).into()]).search().from("a").query(),
...
QueryBuilder::search().to("a").from("b").query(), 
