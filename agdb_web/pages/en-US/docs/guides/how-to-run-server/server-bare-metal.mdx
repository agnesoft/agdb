...
```shell
cargo install agdb_server --features tls # the `tls` feature is optional and can be omitted if TLS support is not needed
```

<Callout>
    On all platforms the feature `tls` requires `CMake`. On Windows it requires additionally MSVC toolchain. If you do not need TLS simply omit the feature.
</Callout>
...
cargo build --release -p agdb_server --features tls # the `tls` feature is optional and can be omitted if TLS support is not needed
