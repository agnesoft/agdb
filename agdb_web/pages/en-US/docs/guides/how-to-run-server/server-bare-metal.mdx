...
- [OPTIONAL, REQUIRED FOR STUDIO] Node.js: [official source](https://nodejs.org/en/download/) (skip if you do not need the server GUI).

- [OPTIONAL, UNIX ONLY] CMake: [official source](https://cmake.org/download/).

- [OPTIONAL, WINDOWS ONLY, REQUIRED FOR TLS] Microsfot Build Tools (includes CMake): [official source](https://learn.microsoft.com/en-us/cpp/build/building-on-the-command-line).

### Install `agdb_server`

```shell
cargo install agdb_server --features "tls studio"
    # the `tls` feature is optional and can be omitted if TLS support is not needed
    # the `studio` feature is optional can be omitted if you do not need server GUI otherwise you need to build the agdb_api/typescript and agdb_studio projects first to have the required files
```

<Callout>
    On all platforms the feature `tls` requires `CMake`. On Windows it requires
    additionally MSVC toolchain. If you do not need TLS simply omit the feature.
</Callout>
...
# thse GUI builds can be omitted if "studio" features is not used
cd agdb_api/typescrtipt
npm install
npm run build
cd ../../agdb_studio
npm install
...
cargo build --release -p agdb_server --features "tls studio"
    # the `tls` feature is optional and can be omitted if TLS support is not needed
    # the `studio` feature is optional can be omitted if you do not need server GUI
