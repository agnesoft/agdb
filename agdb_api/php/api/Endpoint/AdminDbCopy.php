...
class AdminDbCopy extends \Agnesoft\Agdb\Runtime\Client\BaseEndpoint implements \Agnesoft\Agdb\Runtime\Client\Endpoint
{
    protected $owner;
    protected $db;
    /**
    ...
    {
        return str_replace(['{owner}', '{db}'], [$this->owner, $this->db], '/api/v1/admin/db/{owner}/{db}/copy');
    }
    ...
    protected function transformResponseBody(\Psr\Http\Message\ResponseInterface $response, \Symfony\Component\Serializer\SerializerInterface $serializer, ?string $contentType = null)
    {
        ...
        if (401 === $status) {
            throw new \Agnesoft\Agdb\Exception\AdminDbCopyUnauthorizedException($response);
        }
        if (404 === $status) {
            throw new \Agnesoft\Agdb\Exception\AdminDbCopyNotFoundException($response);
        }
        if (465 === $status) {
            throw new \Agnesoft\Agdb\Exception\AdminDbCopyCustom465Exception($response);
        }
        if (467 === $status) {
            throw new \Agnesoft\Agdb\Exception\AdminDbCopyCustom467Exception($response);
        }
    }
