...
<Callout>
The pepper value (used for additionally hashing the passwords, see the [configuration](#configuration) section) also needs to be the same across the cluster as when sending actions to do with passwords only the hashed values are sent between nodes rather than raw passwords for security reasons. 
</Callout>
