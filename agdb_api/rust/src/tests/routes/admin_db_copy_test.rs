...
pub async fn copy() -> Result<(), TestError> {
...
#[cfg_attr(feature = "api", agdb::test_def())]
pub async fn copy_to_different_user() -> Result<(), TestError> {
    let mut server = TestServer::new().await?;
    let owner = &next_user_name();
...
#[cfg_attr(feature = "api", agdb::test_def())]
pub async fn copy_target_exists() -> Result<(), TestError> {
    let mut server = TestServer::new().await?;
    let owner = &next_user_name();
    let db = &next_db_name();
    let db2 = &next_db_name();
...
pub fn test_defs() -> Vec<agdb::type_def::Type> {
    vec![
        __copy_type_def(),
        __copy_to_different_user_type_def(),
        __copy_target_exists_type_def(),
