...
description: "Designing efficient agdb schemas and queries for realistic workloads"
---
...
This condition makes sure only `liked` property is actually followed and will stop immediately after at possibly the destination node. Additionally, it will not follow unless `liked` property existed on an adjacent edge.
