import useEdge, { type Edge } from "@/composables/graph/composable/edge";
...
describe("useEdge with undefined from or to", () => {
    let node: Node;
    ...
    it("should return zero length if the from node is undefined", () => {
        const edge = useEdge({ id: 1, from: undefined, to: node, values: {} });
        expect(edge.getLength()).toBe(0);
    });

    it("should return zero length if the to node is undefined", () => {
        const edge = useEdge({ id: 1, from: node, to: undefined, values: {} });
        expect(edge.getLength()).toBe(0);
    });
