---
title: "Kubernetes"
description: "Deploy single agdb_server on Kubernetes: Service, Secrets, ConfigMap, StatefulSet, and TLS"
---

The `agdb_server` can be run in Kubernetes using official Docker image. Optionally you can [build the image](https://github.com/agnesoft/agdb/blob/main/agdb_server/containerfile) yourself and host it at the repository of your choosing. Please refer to the [server-docker](/docs/guides/how-to-run-server/server-docker) guide for available images.
...
DNS name of the server used in its configuration `address` field. E.g.
