...
async fn delete() -> anyhow::Result<()> {
    ...
    assert_eq!(
        server
            .post(ADMIN_DB_DELETE_URI, &rem, &server.admin_token)
            .await?
            .0,
        204
    );
...
async fn db_not_found() -> anyhow::Result<()> {
    ...
    assert_eq!(
        server
            .post(ADMIN_DB_DELETE_URI, &db, &server.admin_token)
            .await?
            .0,
        466
    );
...
async fn no_admin_token() -> anyhow::Result<()> {
    ...
    assert_eq!(
        server.post(ADMIN_DB_DELETE_URI, &db, NO_TOKEN).await?.0,
        401
    );
    Ok(())
}
